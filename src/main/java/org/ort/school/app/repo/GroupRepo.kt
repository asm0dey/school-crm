...
class GroupRepo @Inject constructor(private val ctx: DSLContext) {
    fun listGroupsWithChildren(): Map<Long, List<GroupInfo>> {
        ...
        return ctx
                ...
                .from(GROUP)
                .innerJoin(GROUP_STUDENT).on(GROUP_STUDENT.GROUP_ID.eq(GROUP.ID))
                .innerJoin(STUDENT).on(GROUP_STUDENT.STUDENT_ID.eq(STUDENT.ID))
                .innerJoin(GRADE).on(STUDENT.GRADE_ID.eq(GRADE.ID))
                .fetch { GroupInfo(it[GROUP.ID], it[GROUP.NAME], it[STUDENT.ID], it[fullName], it[gradeName]) }
                .groupBy { it.id }
