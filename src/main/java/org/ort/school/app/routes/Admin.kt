...
class Admin @Inject constructor(
    ...
    @GET
    @Path("/users")
    fun getUsers(@Local profile: CommonProfile): users {
        if (!profile.roles.contains("admin")) throw Err(403)
        return views.priv.admin.users().users(userService.listUsers())
    }

    @GET
    @Path("/degrees")
    fun degrees(@Local profile: CommonProfile): degrees {
        if (!profile.roles.contains("admin")) throw Err(403)
        ...
        return views.priv.admin.degrees()
                .degrees(degrees)
    ...
    @Path("/degrees")
    @POST
    fun createDegree(degree: DegreeDTO, @Local profile: CommonProfile): degrees {
        if (!profile.roles.contains("admin")) throw Err(403)
